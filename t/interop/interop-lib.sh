...
build_version() {
  ...
  for config in config.mak config.mak.autogen config.status; do
    if test -e "$INTEROP_ROOT/../../$config"; then
...
generate_wrappers() {
  mkdir -p .bin \
    ...
    && write_script .bin/git <<-\EOF && PATH=$(pwd)/.bin:$PATH
...
if ! DIR_A=$(build_version "$VERSION_A" "$MAKE_OPTS_A") \
  || ! DIR_B=$(build_version "$VERSION_B" "$MAKE_OPTS_B"); then
