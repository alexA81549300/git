...
should_create_test_file() {
  ...
  case $file in
    ...
    */)
    ...
    # details.
    *\\*)
      if ! test_have_prereq BSLASHPSPEC; then
        return 1
      fi
...
match_with_ls_files() {
  ...
  if test "$match_expect" = 'E'; then
    if test -e .git/created_test_file; then
