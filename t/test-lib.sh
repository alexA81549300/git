...
if test "$TEST_DIRECTORY" = "$GIT_BUILD_DIR"; then
  echo "PANIC: Running in a $TEST_DIRECTORY that doesn't end in '/t'?" >&2
  exit 1
fi
if test -f "$GIT_BUILD_DIR/GIT-BUILD-DIR"; then
...
if test -n "$TEST_OUTPUT_DIRECTORY" && test -z "$TEST_OUTPUT_DIRECTORY_OVERRIDE"; then
  TEST_OUTPUT_DIRECTORY_OVERRIDE=$TEST_OUTPUT_DIRECTORY
fi

if test ! -f "$GIT_BUILD_DIR"/GIT-BUILD-OPTIONS; then
...
if test -z "$TEST_OUTPUT_DIRECTORY"; then
...
if test -n "${TEST_OUTPUT_DIRECTORY_OVERRIDE}"; then
  TEST_OUTPUT_DIRECTORY="${TEST_OUTPUT_DIRECTORY_OVERRIDE}"
fi

# Disallow the use of abbreviated options in the test suite by default
if test -z "${GIT_TEST_DISALLOW_ABBREVIATED_OPTIONS}"; then
...
mark_option_requires_arg() {
  if test -n "$opt_required_arg"; then
...
if test -n "$valgrind_only"; then
  test -z "$valgrind" && valgrind=memcheck
  test -z "$verbose" && verbose_only="$valgrind_only"
elif test -n "$valgrind"; then
...
if test "$GIT_TEST_STRESS_STARTED" = "done"; then
  : # Don't stress test again.
elif test -n "$stress"; then
  if test -n "$stress_jobs"; then
    job_count=$stress_jobs
  elif test -n "$GIT_TEST_STRESS_LOAD"; then
    job_count="$GIT_TEST_STRESS_LOAD"
  elif job_count=$(getconf _NPROCESSORS_ONLN 2>/dev/null) \
    && test -n "$job_count"; then
  ...
  job_pids=
  job_nr=0
  while test $job_nr -lt "$job_count"; do
    (
  ...
  if test -f "$stressfail"; then
    stress_exit=1
    echo "Log(s) of failed test run(s):"
    for failed_job_nr in $(sort -n "$stressfail"); do
...
if test "$GIT_TEST_TEE_STARTED" = "done"; then
  : # do not redirect again
elif test -n "$tee"; then
  mkdir -p "$TEST_RESULTS_DIR"
...
if test -n "$trace" && test -n "$test_untraceable"; then
...
if test -n "$trace" && test -z "$verbose_log"; then
  verbose=t
fi
...
if test -n "${GIT_TEST_INDEX_VERSION:+isset}"; then
  GIT_INDEX_VERSION="$GIT_TEST_INDEX_VERSION"
  export GIT_INDEX_VERSION
fi

if test -n "$GIT_TEST_PERL_FATAL_WARNINGS"; then
...
if test -n "$HARNESS_ACTIVE"; then
  if test "$verbose" = t || test -n "$verbose_only"; then
...
if test "$verbose_log" = "t"; then
  exec 3>>"$GIT_TEST_TEE_OUTPUT_FILE" 4>&3
elif test "$verbose" = "t"; then
  exec 4>&2 3>&1
else
...
test_failure_() {
  ...
  local pfx=""
  if test -n "$invert_exit_code"; then # && test -n "$HARNESS_ACTIVE"
    pfx="# TODO induced breakage (--invert-exit-code):"
  fi
  say_color error "not ok $test_count - ${pfx:+$pfx }$1"
  shift
  printf '%s\n' "$*" | sed -e 's/^/#	/'
  if test -n "$immediate"; then
    say_color error "1..$test_count"
    if test -n "$invert_exit_code"; then
      finalize_test_output
      _invert_exit_code_failure_end_blurb
      GIT_EXIT_OK=t
      exit 0
...
maybe_setup_verbose() {
  test -z "$verbose_only" && return
  if match_pattern_list $test_count "$verbose_only"; then
    exec 4>&2 3>&1
...
maybe_setup_valgrind() {
  test -z "$GIT_VALGRIND" && return
  if test -z "$valgrind_only"; then
    GIT_VALGRIND_ENABLED=t
    return
  fi
  GIT_VALGRIND_ENABLED=
  if match_pattern_list $test_count "$valgrind_only"; then
    GIT_VALGRIND_ENABLED=t
  fi
}
...
test_eval_() {
  ...
  if test "$test_eval_ret_" != 0 && want_trace; then
    say_color error "error: last command exited with \$?=$test_eval_ret_" >&4
  fi
...
test_run_() {
  ...
  if test -z "$immediate" || test $eval_ret = 0 \
    || test -n "$expecting_failure" && test "$test_cleanup" != ":"; then
    setup_malloc_check
...
test_finish_() {
  ...
  maybe_teardown_valgrind
  maybe_teardown_verbose
  if test -n "$GIT_TEST_TEE_OFFSET"; then
...
test_skip() {
  to_skip=
  skipped_reason=
  if match_pattern_list $this_test.$test_count "$GIT_SKIP_TESTS"; then
    to_skip=t
    skipped_reason="GIT_SKIP_TESTS"
  fi
  if test -z "$to_skip" && test -n "$run_list" \
    && ! match_test_selector_list '--run' "$1" $test_count "$run_list"; then
    to_skip=t
    skipped_reason="--run"
  fi
  if test -z "$to_skip" && test -n "$test_prereq" \
    && ! test_have_prereq "$test_prereq"; then
    to_skip=t
...
check_test_results_san_file_() {
  if ! check_test_results_san_file_has_entries_; then
    return
  fi \
    && say_color error "$(cat "$TEST_RESULTS_SAN_FILE".*)" \
    && if test "$test_failure" = 0; then
...
test_done() {
  ...
  case "$test_failure" in
    0)
      ...
      if test -n "$stress" && test -n "$invert_exit_code"; then
      ...
      elif test -z "$debug" && test -n "$remove_trash"; then
      ...
      if test -z "$skip_all" && test -n "$invert_exit_code"; then
...
if test -n "$valgrind"; then
  make_symlink() {
    test -h "$2" \
      && test "$1" = "$(readlink "$2")" || {
      # be super paranoid
      if mkdir "$2".lock; then
        rm -f "$2" \
          && ln -s "$1" "$2" \
          && rm -r "$2".lock
      else
        while test -d "$2".lock; do
  ...
  make_valgrind_symlink() {
    ...
    if test -x "$symlink_target" \
      ...
      && test "#!" != "$(test_copy_bytes 2 <"$symlink_target")"; then
  ...
  for file in $GIT_BUILD_DIR/git* $GIT_BUILD_DIR/t/helper/test-*; do
  ...
  if test -n "$no_bin_wrappers"; then
    with_dashes=t
  else
...
if test -n "$valgrind" \
  ...
  setup_malloc_check() {
    ...
    if test -n "$_USE_GLIBC_TUNABLES"; then
      g=
      LD_PRELOAD=$_USE_GLIBC_PRELOAD
      for t in \
        glibc.malloc.check=1 \
        glibc.malloc.perturb=165; do
        g="${g#:}:$t"
      done
      GLIBC_TUNABLES=$g
...
if test -z "$GIT_TEST_CMP"; then
  if test -n "$GIT_TEST_CMP_USE_COPIED_CONTEXT"; then
    GIT_TEST_CMP="$DIFF -c"
  else
    GIT_TEST_CMP="$DIFF -u"
  fi
...
if ! test -x "$GIT_BUILD_DIR"/t/helper/test-tool$X; then
...
this_test=${0##*/}
this_test=${this_test%%-*}
if match_pattern_list "$this_test" "$GIT_SKIP_TESTS"; then
  say_color info "skipping test $this_test altogether" >&3
  skip_all="skip all tests in $this_test"
  test_done
fi

if test -n "$SANITIZE_LEAK"; then
  rm -rf "$TEST_RESULTS_SAN_DIR"
  if ! mkdir -p "$TEST_RESULTS_SAN_DIR"; then
...
if test "${GIT_TEST_CHAIN_LINT:-1}" != 0 \
  && test "${GIT_TEST_EXT_CHAIN_LINT:-1}" != 0; then
...
if test -n "$GIT_TEST_FAIL_PREREQS"; then
  if test_bool_env GIT_TEST_FAIL_PREREQS false; then
