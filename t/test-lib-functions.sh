...
test_commit_bulk() {
  ...
  add_from=
  if git -C "$indir" rev-parse --quiet --verify "$ref"; then
    add_from=t
  fi

  while test "$total" -gt 0; do
    if test -z "$notick"; then
      test_tick
    fi \
      && echo "commit $ref"
...
test_hook() {
  setup= \
    ...
    && if test -z "$clobber"; then
      test_path_is_missing "$hook_file"
    fi \
    && if test -z "$setup$clobber"; then
...
test_set_prereq() {
  if test -n "$GIT_TEST_FAIL_PREREQS_INTERNAL"; then
    case "$1" in
  ...
  for prerequisite; do
        ...
        case " $lazily_testable_prereq " in
          *" $prerequisite "*)
            eval "script=\$test_prereq_lazily_$prerequisite" \
              && if test_run_lazy_prereq_ "$prerequisite" "$script"; then
    ...
    case "$satisfied_this_prereq,$negative_prereq" in
      ...
      *)
        ...
        if test -n "$GIT_TEST_REQUIRE_PREREQ"; then
...
test_expect_failure() {
  ...
  if ! test_skip "$@"; then
    ...
    if test_run_ "$test_body" expecting_failure; then
...
test_expect_success() {
  ...
  if ! test_skip "$@"; then
    ...
    if test_run_ "$test_body" \
      && ! check_test_results_san_file_has_entries_; then
      test_ok_ "$1"
    else
...
test_dir_is_empty() {
  test "$#" -ne 1 && BUG "1 param"
  test_path_is_dir "$1" \
    && if test -n "$(ls -a1 "$1" | grep -E -v '^\.\.?$')"; then
...
test_must_fail() {
  ...
  if ! test_must_fail_acceptable "$@"; then
    echo >&7 "test_must_fail: only 'git' is allowed: $*"
    return 1
  fi
  "$@" 2>&7
  exit_code=$?
  if test $exit_code -eq 0 && ! list_contains "$_test_ok" success; then
    echo >&4 "test_must_fail: command succeeded: $*"
    return 1
  elif test_match_signal 13 $exit_code && list_contains "$_test_ok" sigpipe; then
    return 0
  elif test $exit_code -gt 129 && test $exit_code -le 192; then
    echo >&4 "test_must_fail: died by signal $(($exit_code - 128)): $*"
    return 1
  elif test $exit_code -eq 127; then
    echo >&4 "test_must_fail: command not found: $*"
    return 1
  elif test $exit_code -eq 126; then
...
test_cmp_fspath() {
  ...
  if test true != "$(git config --get --type=bool core.ignorecase)"; then
    return 1
  fi
...
test_seq() {
  ...
  while test "$test_seq_counter__" -le "$2"; do
...
test_match_signal() {
  if test "$2" = "$((128 + $1))"; then
    # POSIX
    return 0
  elif test "$2" = "$((256 + $1))"; then
    # ksh
    return 0
  fi
  return 1
    ...
    && while read tag rest; do
      case $tag in
        ...
        *)
        ...
        && k="${rest%:*}" \
        && v="${rest#*:}" \
        && if ! expr "$k" : '[a-z0-9][a-z0-9]*$' >/dev/null; then
...
test_region() {
  ...
  if test $exitcode != $expect_exit; then
    return 1
  fi
