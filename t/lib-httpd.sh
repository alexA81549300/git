...
if ! test_have_prereq LIBCURL; then
  skip_all='skipping test, git built without http support'
  test_done
fi

if test -n "$NO_EXPAT" && test -n "$LIB_HTTPD_DAV"; then
  skip_all='skipping test, git built without expat support'
  test_done
fi

if ! test_bool_env GIT_TEST_HTTPD true; then
...
for DEFAULT_HTTPD_PATH in '/usr/sbin/httpd' \
  '/usr/sbin/apache2' \
  "$(command -v httpd)" \
  "$(command -v apache2)"; do
  if test -n "$DEFAULT_HTTPD_PATH" && test -x "$DEFAULT_HTTPD_PATH"; then
    break
  fi
done

if test -x "$DEFAULT_HTTPD_PATH"; then
...
for DEFAULT_HTTPD_MODULE_PATH in '/usr/libexec/apache2' \
  ...
  '/usr/libexec/httpd' \
  '/usr/lib/apache2' \
  "${DETECTED_HTTPD_ROOT:+${DETECTED_HTTPD_ROOT}/modules}"; do
  if test -n "$DEFAULT_HTTPD_MODULE_PATH" && test -d "$DEFAULT_HTTPD_MODULE_PATH"; then
    break
  fi
done
...
GIT_VALGRIND=$GIT_VALGRIND
export GIT_VALGRIND
GIT_VALGRIND_OPTIONS=$GIT_VALGRIND_OPTIONS
export GIT_VALGRIND_OPTIONS
GIT_TEST_SIDEBAND_ALL=$GIT_TEST_SIDEBAND_ALL
export GIT_TEST_SIDEBAND_ALL
GIT_TRACE=$GIT_TRACE
export GIT_TRACE
...
if test -n "$HTTPD_VERSION_MAJOR"; then
  if test -z "$LIB_HTTPD_MODULE_PATH"; then
    if ! test "$HTTPD_VERSION_MAJOR" -eq 2 \
      || ! test "$HTTPD_VERSION_MINOR" -ge 4; then
      test_skip_or_die GIT_TEST_HTTPD \
        "at least Apache version 2.4 is required"
    fi
    if ! test -d "$DEFAULT_HTTPD_MODULE_PATH"; then
      test_skip_or_die GIT_TEST_HTTPD \
...
if test -n "$LIB_HTTPD_DAV" && test -f /etc/os-release; then
...
prepare_httpd() {
  ...
  if test -n "$LIB_HTTPD_DAV" || test -n "$LIB_HTTPD_SVN"; then
    HTTPD_PARA="$HTTPD_PARA -DDAV"

    if test -n "$LIB_HTTPD_SVN"; then
...
enable_cgipassauth() {
  ...
  if test "$HTTPD_VERSION_MAJOR" = 2 \
    ...
    && test "$(echo $HTTPD_VERSION | cut -d. -f3)" -lt 13; then
...
check_access_log() {
  sort "$1" >"$1".sorted \
    ...
    && if ! test_cmp "$1".sorted access.log.sorted; then
