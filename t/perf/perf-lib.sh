...
if test -n "$GIT_TEST_INSTALLED" && test -z "$PERF_SET_GIT_TEST_INSTALLED"; then
...
test_perf_create_repo_from() {
  ...
  (
    ...
    if test "$source_git" != "$common_dir"; then
    ...
    && (
      cd "$repo" \
        ...
        && if test_bool_env GIT_PERF_USE_SCALAR false; then
...
exit $ret' >&3 2>&4
  ...
  if test $eval_ret = 0 || test -n "$expecting_failure"; then
...
test_perf_() {
  ...
  for i in $(test_seq 1 $GIT_PERF_REPEAT_COUNT); do
    if test -n "$test_perf_setup_"; then
      say >&3 "setup: $test_perf_setup_"
      if ! test_eval_ $test_perf_setup_; then
