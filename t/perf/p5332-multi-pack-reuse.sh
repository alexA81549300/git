...
find_pack() {
  for idx in $packdir/pack-*.idx; do
    if git show-index <$idx | grep -q "$1"; then
      basename $idx
    fi || return 1
  done
}
...
repack_into_n_chunks() {
  ...
  for rev in $(git rev-list --all | awk "NR % $sz == 0" | tac); do
  ...
  git pack-objects --revs --all --honor-pack-keep --delta-base-offset \
    ...
    for f in $(comm -12 packs.before packs.after); do
      rm -f "$f" || return 1
    done
  rm -fr $packdir/*.keep
}

for nr_packs in 1 10 100; do
