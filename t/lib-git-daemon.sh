...
if ! test_bool_env GIT_TEST_GIT_DAEMON true; then
  skip_all="git-daemon testing disabled (unset GIT_TEST_GIT_DAEMON to enable)"
  test_done
fi

if test_have_prereq !PIPE; then
...
start_git_daemon() {
  ...
  if test -z "$registered_stop_git_daemon_atexit_handler"; then
  ...
  GIT_DAEMON_PID=$!
  {
    ...
    if test x"$(expr "$line" : "\[[0-9]*\] \(.*\)")" != x"Ready to rumble"; then
      kill "$GIT_DAEMON_PID"
      wait "$GIT_DAEMON_PID"
...
stop_git_daemon() {
  ...
  ret=$?
  if ! test_match_signal 15 $ret; then
...
# EOF) to stdout.
fake_nc() {
  if ! test_declared_prereq FAKENC; then
