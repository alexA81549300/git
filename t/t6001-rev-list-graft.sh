...
check() {
  ...
  for a; do
    ...
    if test "$type" = basic; then
      ...
      if test "z$child" != z; then
        echo "$child $a"
      fi
      child="$a"
    fi
  done >test.expect
  if test "$type" != basic && test "z$child" != z; then
    echo >>test.expect $child
  fi
  if test $type = basic; then
    git rev-list $arg >test.actual
  elif test $type = parents; then
    git rev-list --parents $arg >test.actual
  elif test $type = parents-raw; then
...
for type in basic parents parents-raw; do
