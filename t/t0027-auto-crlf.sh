...
create_NNO_MIX_files() {
  for crlf in false true input; do
    for attr in "" auto text -text; do
      for aeol in "" lf crlf; do
...
commit_check_warn() {
  ...
  create_gitattributes "$attr" \
    && for f in LF CRLF LF_mix_CR CRLF_mix_LF LF_nul CRLF_nul; do
      fname=${pfx}_$f.txt \
        && cp $f $fname \
...
checkout_files() {
  ...
  pfx=eol_${ceol}_crlf_${crlf}_attr_${attr}_ \
    && for f in LF CRLF LF_mix_CR CRLF_mix_LF LF_nul; do
...
for crlf in true false input; do
