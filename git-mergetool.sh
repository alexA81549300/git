...
mergetool_tmpdir_init() {
  if test "$(git config --bool mergetool.writeToTemp)" != true; then
    MERGETOOL_TMPDIR=.
    return 0
  fi
  if MERGETOOL_TMPDIR=$(mktemp -d -t "git-mergetool-XXXXXX" 2>/dev/null); then
    return 0
  fi
...
checkout_staged_file() {
  ...
  if test $? -eq 0 && test -n "$tmpfile"; then
...
merge_file() {
  ...
  if enabled=$(git config --type=bool "$tool_config"); then
    # The user has a specific preference for a specific tool and no
    # other preferences should override that.
    :
  elif enabled=$(git config --type=bool "$global_config"); then
    ...
    # not use it anywhere.
    if test "$enabled" = true && hide_resolved_enabled; then
      enabled=true
    else
      enabled=false
    fi
  ...
  describe_file "$local_mode" "local" "$LOCAL"
  describe_file "$remote_mode" "remote" "$REMOTE"
  if test "$guessed_merge_tool" = true || test "$prompt" = true; then
...
main() {
  ...
  if test -z "$merge_tool"; then
    ...
    if test $subshell_exit_status = 1; then
      guessed_merge_tool=true
    elif test $subshell_exit_status -gt 1; then
  ...
  if test $# -eq 0 && test -e "$GIT_DIR/MERGE_RR"; then
