...
if test "$#" -ne 1 && test "$#" -ne 3 || test -z "$1"; then
...
if ! git rev-parse --quiet --verify "${baseCommit}"; then
  echo "Invalid <BASE_COMMIT> '${baseCommit}'"
  exit 1
fi

while read dash sha etc; do
  case "${dash}" in
    "---") # Line contains commit information.
      if test -z "${goodParent}"; then
