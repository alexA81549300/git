...
if test true = "$GITHUB_ACTIONS"; then
  begin_group() {
    need_to_end_group=t
    echo "::group::$1" >&2
    set -x
...
# message.
save_good_tree() {
  if ! is_usable_git_repository .; then
    return
  fi
...
skip_good_tree() {
  ...
  if ! good_tree_info="$(grep "^$(git rev-parse $CI_COMMIT^{tree}) " "$good_trees_file")"; then
  ...
  echo "$good_tree_info" | {
    ...
    if test "$CI_JOB_ID" = "$prev_good_job_id"; then
      cat <<-EOF
...
create_failed_test_artifacts() {
  ...
  for test_exit in "${TEST_OUTPUT_DIRECTORY:-t}"/test-results/*.exit; do
...
case "$distro" in
  ubuntu-*)
    ...
    # releases.
    if test "$distro" = "ubuntu-20.04"; then
