...
#     )
set_reflog_action() {
  if [ -z "${GIT_REFLOG_ACTION:+set}" ]; then
    GIT_REFLOG_ACTION="$*"
...
require_work_tree_exists() {
  if test "z$(git rev-parse --is-bare-repository)" != zfalse; then
    program_name=$0
