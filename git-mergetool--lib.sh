...
$(list_tool_variants)"
    ...
    for toolname in $variants; do
      if setup_tool "$toolname" 2>/dev/null \
        && (eval "$condition" "$toolname"); then
    ...
    if test -n "$preamble" && test -n "$not_found_msg"; then
      printf "%s\n" "$not_found_msg"
    fi
...
get_gui_default() {
  ...
  if test "$GUI_DEFAULT_CONFIG_LCASE" = "auto"; then
    ...
    if test $subshell_exit_status -ne 0; then
...
gui_mode() {
  if test -z "$GIT_MERGETOOL_GUI"; then
...
setup_tool() {
  ...
  if test -f "$MERGE_TOOLS_DIR/$tool"; then
    . "$MERGE_TOOLS_DIR/$tool"
  elif test -f "$MERGE_TOOLS_DIR/${tool%[0-9]}"; then
  ...
  if ! list_tool_variants | grep -q "^$tool$"; then
    echo "error: unknown tool variant '$tool'" >&2
    return 1
  fi

  if merge_mode && ! can_merge; then
    echo "error: '$tool' can not be used to resolve merges" >&2
    return 1
  elif diff_mode && ! can_diff; then
...
trust_exit_code() {
  if git config --bool "mergetool.$1.trustExitCode"; then
    : # OK
  elif exit_code_trustable; then
    echo true
  else
    echo false
  fi
...
run_merge_cmd() {
  ...
  if test "$mergetool_trust_exit_code" = "true"; then
    merge_cmd "$1"
  else
    touch "$BACKUP"
    merge_cmd "$1"
...
get_merge_tool() {
  ...
  subshell_exit_status=$?
  if test $subshell_exit_status -gt "1"; then
    exit $subshell_exit_status
  fi
...
mergetool_find_win32_cmd() {
  ...
  for directory in $(env | grep -Ei '^PROGRAM(FILES(\(X86\))?|W6432)=' \
    | cut -d '=' -f 2- | sort -u); do
    if test -n "$directory" && test -x "$directory/$sub_directory/$executable"; then
