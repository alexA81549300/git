...
cache_get() {
  for oldrev in "$@"; do
    if test -r "$cachedir/$oldrev"; then
...
check_parents() {
  ...
  for miss in $missed; do
    if ! test -r "$cachedir/notree/$miss"; then
...
process_subtree_split_trailer() {
  ...
  if ! sub="$(git rev-parse --verify --quiet "$b^{commit}")"; then
...
find_existing_splits() {
  ...
  if test -n "$arg_split_ignore_joins"; then
...
add_msg() {
  ...
  latest_old="$2"
  latest_new="$3"
  if test -n "$arg_addmerge_message"; then
    commit_message="$arg_addmerge_message"
  else
    commit_message="Add '$dir/' from commit '$latest_new'"
  fi
  if test -n "$arg_split_rejoin"; then
...
copy_or_skip() {
  ...
  for parent in $newparents; do
    ...
    if test "$ptree" = "$tree"; then
      ...
      if test -n "$identical"; then
        # if a previous identical parent was found, check whether
        # one is already an ancestor of the other
        mergebase=$(git merge-base $identical $parent)
        if test "$identical" = "$mergebase"; then
          # current identical commit is an ancestor of parent
          identical="$parent"
        elif test "$parent" != "$mergebase"; then
    ...
    for gp in $gotparents; do
      if test "$gp" = "$parent"; then
        is_new=
        break
      fi
    done
    if test -n "$is_new"; then
      gotparents="$gotparents $parent"
      p="$p -p $parent"
    fi
  done

  if test -n "$identical" && test -n "$nonidentical"; then
...
should_ignore_subtree_split_commit() {
  assert test $# = 1
  local rev="$1"
  if test -n "$(git log -1 --grep="git-subtree-dir:" $rev)"; then
    if test -z "$(git log -1 --grep="git-subtree-mainline:" $rev)" \
      && test -z "$(git log -1 --grep="git-subtree-dir: $arg_prefix$" $rev)"; then
      return 0
    fi
  fi
  return 1
...
cmd_add_commit() {
  ...
  if test -z "$arg_split_rejoin"; then
  ...
  if test -n "$arg_addmerge_squash"; then
...
cmd_split() {
  ...
  eval "$grl" \
    | while read rev parents; do
      if should_ignore_subtree_split_commit "$rev"; then
        continue
      fi
      parsedparents=''
      for parent in $parents; do
        if ! should_ignore_subtree_split_commit "$parent"; then
  ...
  if test -n "$arg_split_branch"; then
    if rev_exists "refs/heads/$arg_split_branch"; then
      if ! git merge-base --is-ancestor "$arg_split_branch" "$latest_new"; then
...
cmd_push() {
  ...
  if test -e "$dir"; then
    ...
    refspec=${2#+}
    remoteref=${refspec#*:}
    if test "$remoteref" = "$refspec"; then
