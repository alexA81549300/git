...
test_check_precond() {
  ...
  if ! test -d "$WIKI_DIR_INST/$WIKI_DIR_NAME"; then
...
wiki_reset() {
  ...
  if ! test -d "../$FILES_FOLDER_DB"; then
    error "No wiki database at ../$FILES_FOLDER_DB, not installed yet?"
  fi
  if ! test -d "../$FILES_FOLDER_POST_INSTALL_DB"; then
