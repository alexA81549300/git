...
if test "z$branch_name" = "z$headref" \
  || ! git config "branch.$branch_name.description" >/dev/null; then
  branch_name=
fi
...
if test -z "$ref"; then
  ...
  status=1
elif test "$local_sha1" != "$remote_sha1"; then
...
if test "$ref" = "refs/tags/$pretty_remote"; then
...
----------------------------------------------------------------' $headrev \
  && if test $(git cat-file -t "$head") = tag; then
