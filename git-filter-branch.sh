...
if test -z "$FILTER_BRANCH_SQUELCH_WARNING$GIT_TEST_DISALLOW_ABBREVIATED_OPTIONS"; then
  cat <<EOF
...
while read ref; do
  ...
  if git rev-parse --verify "$ref"^0 >/dev/null 2>&1; then
    echo "$ref"
  else
  ...
  case "$filter_subdir" in
    ...
    *)
      ...
      err=$(GIT_ALLOW_NULL_SHA1=1 \
        ...
        if ! git rev-parse -q --verify $commit:"$filter_subdir"; then
          rm -f "$GIT_INDEX_FILE"
        else
          echo >&2 "$err"
          false
...
if test "$remap_to_ancestor" = t; then
  while read ref; do
    ...
    | while read sha1 type ref; do
      ...
      if [ "$type" = "tag" ]; then
        ...
        if git cat-file tag "$ref" \
          | grep '^-----BEGIN PGP SIGNATURE-----' >/dev/null 2>&1; then
